...
o	System.IAsyncDisposable, 
o	System.Collections.Generic.IAsyncEnumerable<T> 
o	System.Collections.Generic.IAsyncEnumerator<T> .
...
•	Adicionar Métodos à Interface Padrão	•	Add Methods to the Standard Interface
•	Fornecer Parâmetros à Interface Padrão	•	Provide Parameters to the Standard Interface
•	Estenda a implementação padrão	•	Extend the standard implementation
Aprimoramentos de correspondência de padrões	Pattern matching enhancements
•	expressões Switch	•	Switch expressions
•	Padrões de propriedade	•	Property patterns
...
•	Escolher uma estratégia para tipos de referência anuláveis	•	Choose a strategy for nullable reference types
...
•	Operador de Coalescência Nula ??	•	Null Coalescence Operator ??
...
•	System.Span<T> ou System.ReadOnlySpan<T>	•	System.Span <T> or System.ReadOnlySpan <T>
...
o	sbyte, byte, short, ushort, int, uint, long, ulong, char, float, double, decimal ou bool
o	Qualquer tipo de Enumeração
o	Qualquer tipo de ponteiro (int*, char*, int*[], void*, etc). O tipo especificado antes do * em um tipo de ponteiro é chamado de tipo referent. Somente um tipo não gerenciado pode ser um tipo referent. Os tipos de ponteiro não são herdados de object e não há nenhuma conversão entre tipos de ponteiro e object.
o	Qualquer tipo de struct definido pelo usuário que contém campos de tipos não gerenciados somente e, em C# 7,3 e anterior, não é um tipo construído (um tipo que inclui pelo menos um argumento de tipo)
...
Configurar com C # 9 é basicamente o mesmo que configurar com .NET Preview 5. E, felizmente para você, temos um guia brilhante aqui: https://dotnetcoretutorials.com/2020/05/22/ Getting-started-with-net-5-and-the-msbuild-net-5-cliffnotes/. Mas para as notas precipitadas desse post:
...
•	Expressões With	•	With expressions
...
•	sobrecargas Equals	•	Equals overloads
...
•	Registros Posicionais	•	Positional Records
...
•	Inteiros de tamanho nativo	•	Native sized integers
•	Ponteiros de função	•	Function pointers
•	Suprimir emissão do sinalizador localsinit	•	Suppress emitting localsinit flag
Aprimoramento da tipagem	Typing improvement
•	Expressões new com Target-typing	•	Target-typed new expressions
•	Expressões condicionais Target-typing	•	Target-typed conditional expressions
•	Tipos de retorno covariantes	•	Covariant return types
...
•	Inicializadores de módulo	•	Module initializers
•	Novos recursos para métodos parciais	•	New features for partial methods
...
o	Se o registro for selado, nenhum modificador adicional será adicionado ao método "clone" (o que significa que não é virtual).
o	Se o registro não estiver selado, o método "clone" é virtual.
•	Para tipos de registro que não são abstratos quando o tipo base não é objeto:
o	Se o registro estiver selado, o método "clone" também será selado.
o	Se o registro não estiver selado, o método "clone" será substituído.
...
Partindo de um código C# 8 switch:.
...
o	Remove a dependência de qualquer lógica especial de empacotamento
•	Não ser chamado de código gerenciado
o	Limita os cenários que precisam ser tratados (por exemplo, nenhuma chamada por meio de reflexão), permitindo que o foco permaneça na redução do custo de chamar a função gerenciada a partir do código nativo
...
o	Isso significa que a acessibilidade efetiva do método deve ser internal ou public .
o	Isso também significa que o método não pode ser uma função local.
